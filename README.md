...
После `Play` игрок попадает в сгенерированный оккультный командный хаб: каменная ритуальная платформа, газовые фонари, стол с картой района, прототип башни и светящиеся `Врата обороны`.

- Нажми `Начать оборону`, чтобы закрыть хаб и запустить tower-defense бой.
- Если демо-автоматизация не нажала кнопку, сервер запустит бой сам после короткой витрины хаба, чтобы проверочные видео и runtime-маркеры не зависали в лобби.
- Хаб собран только из runtime-generated Roblox primitives: Parts, Cylinders, Spheres, Lights, ParticleEmitters и BillboardGui.
- Сейчас стиль намеренно вдохновлён `Повелителем тайн` для приватного прототипа; перед публичным релизом прямые canon-inspired термины можно переименовать и адаптировать.

После старта боя:

- `ЛКМ` по площадке — выбрать место.
- `B` — построить башню.
- `S` — продать выбранную башню.
- `R` — сброс камеры.
- `ESC` — снять выбор.
- колесо мыши — зум.
- стрелки / правая кнопка мыши — движение камеры.
...
- Нажми `B`, чтобы построить башню на выбранной площадке; если площадка не выбрана, башня появится на следующей свободной площадке.
- Нажми `S`, чтобы продать выбранную башню, или `ESC`, чтобы снять выбор.
- Нажми `R`, чтобы вернуть камеру к тактическому виду; колесо мыши приближает и отдаляет, стрелки двигают обзор, правая кнопка мыши тянет карту.
- Нажми кнопку `Быстрая постройка (B)`, чтобы купить базовую башню на следующей свободной площадке.
...
- Камера остаётся в простом тактическом режиме, но её можно двигать, приближать, отдалять и сбрасывать.
